...
import hvan.qlkh.utils.BlankPlotChart;
import hvan.qlkh.utils.BlankPlotChatRender;
import hvan.qlkh.utils.SeriesSize;
...
public class Chart extends javax.swing.JPanel {
    ...
    public Chart() {
        ...
        blankPlotChart.setBlankPlotChatRender(new BlankPlotChatRender() {
            ...
            public void renderSeries(BlankPlotChart chart, Graphics2D g2, SeriesSize size, int index) {
                double totalSeriesWidth = (double)(SERIES_SIZE * legends.size()) + (double)(SERIES_SPACE * (legends.size() - 1));
                double x = (size.getWidth() - totalSeriesWidth) / 2;
                for (int i = 0; i < legends.size(); i++) {
                    ModelLegend legend = legends.get(i);
                    g2.setColor(legend.getColor());
                    double seriesValues = chart.getSeriesValuesOf(model.get(index).getValues()[i], size.getHeight());
                    g2.fillRect((int) (size.getX() + x), (int) (size.getY() + size.getHeight() - seriesValues), SERIES_SIZE, (int) seriesValues);
                    x += SERIES_SPACE + SERIES_SIZE;
                }
            }
        });
    ...
    private void initComponents() {
        ...
        blankPlotChart = new hvan.qlkh.utils.BlankPlotChart();
    ...
    private hvan.qlkh.utils.BlankPlotChart blankPlotChart;

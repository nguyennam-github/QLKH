...
public final class Main extends javax.swing.JPanel implements ListSelectionListener{
    ...
    private void setPieChart(){
        ...
        if (statistic__PercentStatistic.getSelectedItem().toString().equals(PRICE)) {
            ...
            priceStatistic.put(PRICE_RANGES[0], ProductServices.getInstance().filterByPrice(new BigDecimal("0"), new BigDecimal("1000000")).size());
            priceStatistic.put(PRICE_RANGES[1], ProductServices.getInstance().filterByPrice(new BigDecimal("1000000"), new BigDecimal("5000000")).size());
            priceStatistic.put(PRICE_RANGES[2], ProductServices.getInstance().filterByPrice(new BigDecimal("5000000"), new BigDecimal("10000000")).size());
            priceStatistic.put(PRICE_RANGES[3], ProductServices.getInstance().filterByPrice(new BigDecimal("10000000"), new BigDecimal("20000000")).size());
            priceStatistic.put(PRICE_RANGES[4], ProductServices.getInstance().filterByPrice(new BigDecimal("20000000"), new BigDecimal("-12345")).size());
            int ci = 0;
    ...
    private void initComponents() {
        ...
        statistic__Category.setPreferredSize(new java.awt.Dimension(100, 150));
        ...
        statistic__Manafacturer.setPreferredSize(new java.awt.Dimension(100, 150));
                                    ...
                                    if (toolbar__ManafacturerInput.getText().equals("")){
                                        ...
                                        if (nameCheck == quantityCheck == priceCheck == dateCheck == manafacturerCheck){
                                            ...
                                            switch (response) {
                                                ...
                                                case JOptionPane.YES_OPTION ->  {
                                                    String path = toolbar__ThumbnailInput.getText();
                                                    if (!path.equals(ProductServices.getInstance().getSelectedProduct().getThumbnail())) {
                                                        if (!path.equals("")){
                                                            try {
                                                                File thumbnail = new File(IMAGES_DIR + path.substring(path.lastIndexOf("\\") + 1, path.lastIndexOf(".")) + ".png");
                                                                BufferedImage bufferedImage = ImageIO.read(new File(path));
                                                    ...
                                                    try {
                                                        ...
                                                        ProductDAO.getInstance().update(ProductServices.getInstance().getSelectedProduct().getId(), product);
                                                        appController.setProductsTable(productsTable, ProductServices.getInstance().getProducts());
                                                        resetToolbar(true);
            ...
            if (toolbar__ButtonDelete.isEnabled()){
                ...
                switch (response) {
                    ...
                    case JOptionPane.YES_OPTION ->  {
                        ...
                        ProductDAO.getInstance().delete(ProductServices.getInstance().getSelectedProduct().getId());
                        productsTable.clearSelection();
                        appController.setProductsTable(productsTable, ProductServices.getInstance().getProducts());
                        resetToolbar(true);
    ...
    private void information__ButtonEditMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_information__ButtonEditMouseClicked
        if (information__ButtonEdit.isEnabled() && isUserModify()){
            ...
            switch (response) {
                ...
                case JOptionPane.YES_OPTION ->  {
                    UserServices.getInstance().getSelectedUser().setRead(information__ReadInput.isSelected());
                    UserServices.getInstance().getSelectedUser().setWrite(information__WriteInput.isSelected());
                    UserDAO.getInstance().update(UserServices.getInstance().getSelectedUser().getUsername(), UserServices.getInstance().getSelectedUser());
                    adminController.setUsersTable(usersTable, UserServices.getInstance().getUsers());
                    resetUserEdit();
    ...
    class UserSelectionListener implements ListSelectionListener {
        ...
        public void valueChanged(ListSelectionEvent e) {
            ...
            int row = usersTable.getSelectedRow();
            if (row >= 0) {
                UserServices.setSelectedUser(UserServices.getInstance().findByUsername(usersTable.getModel().getValueAt(row, 1).toString()));
            }
        }
    }
}

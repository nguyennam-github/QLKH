...
public class Main extends javax.swing.JPanel implements ListSelectionListener{
    ...
    private void initPieChart(){
        ...
        if (Statistic__PercentStatistic.getSelectedItem().toString().equals("Danh mục")) {
            ...
            for (int i = 0; i < Toolbar__CategoryInput.getItemCount(); i++) {
                ...
                int value = ProductServices.getInstance().filterByCategory(key).size();
    ...
    private void initOveral(){
        int totalProducts = ProductServices.getInstance().getProducts().size();
        int totalQuantity = 0;
                                            ...
                                            switch (response) {
                                                ...
                                                case JOptionPane.YES_OPTION: {
                                                    try {
                                                        ...
                                                        Product product = ProductDAO.getInstance().update(ProductServices.getInstance().getSelectedProduct(), id, name, category, quantity, price, expDate, manafacturer, thumbnail, description);
                                                        appController.setProductsTable(productsTable, ProductServices.getInstance().getProducts());
                                                        resetToolbar(true);
                                                        Products__Main.remove(ProductServices.getInstance().getProducts().indexOf(product));
                                                        ProductTemplate temp = new ProductTemplate(product);
